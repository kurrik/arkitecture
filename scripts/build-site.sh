...
#
# Two generation steps run before the static files in site/ are published:
...
#      current library.
#   2. Build the WebAssembly module the Examples page loads for live, in-browser
...
#
# Outputs in site/: example *.svg (committed, refreshed here) and
# arkitecture.wasm + wasm_exec.js (build artifacts, git-ignored).
set -euo pipefail
