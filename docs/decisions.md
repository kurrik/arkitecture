...
- A node's resolved layout is its declarations **in source order, last-write-wins** (`@use` expands in place). Two *separate* rules setting the same property on the same node is a **validation error**, not a silent resolution.
